...
import {
  ...
  sortCampaignDetails,
  throwError,
} from "../utils/genericUtils"; // Import utility functions
import {
  validateProjectFacilityResponse,
  validateProjectResourceResponse,
  validateStaffResponse,
} from "../validators/genericValidator"; // Import validation functions
import {
  extractCodesFromBoundaryRelationshipResponse,
  generateFilteredBoundaryData,
  getLocalizedName,
} from "../utils/campaignUtils"; // Import utility functions
import {
...
async function getAutoGeneratedBoundaryCodesHandler(
  boundaryList: any,
  childParentMap: any,
  elementCodesMap: any,
  countMap: any,
  ...
  try {
    ...
    logger.info(
      "Auto Generation of Boundary code begins for the user uploaded sheet"
    );
    const updatedelementCodesMap = await getAutoGeneratedBoundaryCodes(
      boundaryList,
      childParentMap,
      elementCodesMap,
      countMap,
 ...
 */
async function getAutoGeneratedBoundaryCodes(
  boundaryList: any,
  childParentMap: any,
  elementCodesMap: any,
  countMap: any,
      ...
      if (!elementCodesMap.has(element)) {
        ...
        if (
          ...
          if (grandParentCode != null && grandParentCode != undefined) {
            ...
            const parentBoundaryCodeTrimmed =
              lastUnderscoreIndex !== -1
                ? parentBoundaryCode.substring(0, lastUnderscoreIndex)
                : parentBoundaryCode;
            code = generateElementCode(
              countMap.get(parentCode),
              parentBoundaryCodeTrimmed,
              element
            );
          } else {
            code = generateElementCode(
              countMap.get(parentCode),
              elementCodesMap.get(parentCode),
              element
            );
          }
          ...
          elementCodesMap.set(
            element,
            (
              request?.body?.ResourceDetails?.hierarchyType + "_"
            ).toUpperCase() + element.toString().substring(0, 2).toUpperCase()
          );
        }
      } else {
        continue;
 ...
 */
async function createBoundaryEntities(
  request: any,
  boundaryMap: Map<string, string>
) {
  try {
    // Create boundary entities
    const requestBody = { RequestInfo: request.body.RequestInfo } as {
      RequestInfo: any;
      Boundary?: any;
    };
    ...
    const boundaryEntityResponse = await httpRequest(
      config.host.boundaryHost + config.paths.boundaryServiceSearch,
      request.body,
      {
    ...
    const codesFromResponse = boundaryEntityResponse.Boundary.map(
      (boundary: any) => boundary.code
    );
    const codeSet = new Set(codesFromResponse); // Creating a set and filling it with the codes from the response
    Array.from(boundaryMap.entries()).forEach(
      async ([boundaryName, boundaryCode]) => {
    ...
    if (!(boundaries.length === 0)) {
      ...
      const response = await httpRequest(
        `${config.host.boundaryHost}boundary-service/boundary/_create`,
        requestBody,
        {},
        "POST"
      );
      ...
      logger.debug(
        "Boundary entities response: " + getFormattedStringForDebug(response)
      );
    } else {
    ...
    throwError(
      ...
      500,
      "INTERNAL_SERVER_ERROR",
      "Error while Boundary Entity Creation"
    );
  }
}
 ...
 */
async function createBoundaryRelationship(
  request: any,
  boundaryTypeMap: { [key: string]: string } = {},
  modifiedChildParentMap: any,
  localizationMap?: any
) {
  try {
    ...
    const boundaryRelationshipResponse = await httpRequest(
      url,
      request.body,
      params
    );
    const boundaryData =
      boundaryRelationshipResponse?.TenantBoundary?.[0]?.boundary;
    const allCodes = extractCodesFromBoundaryRelationshipResponse(boundaryData);
    let flag = 1;
    for (const [boundaryCode, boundaryType] of Object.entries(
      boundaryTypeMap
    )) {
      const localizedBoundaryType = getLocalizedName(
        boundaryType,
        localizationMap
      );
      if (!allCodes.has(boundaryCode)) {
        ...
        const response = await httpRequest(
          `${config.host.boundaryHost}boundary-service/boundary-relationships/_create`,
          requestBody,
          {},
          "POST",
          undefined,
        ...
        if (
          !response.TenantBoundary ||
          !Array.isArray(response.TenantBoundary) ||
          response.TenantBoundary.length === 0
        ) {
          throwError("BOUNDARY", 500, "BOUNDARY_RELATIONSHIP_CREATE_ERROR");
        }
        logger.info(
          `Boundary relationship created for boundaryType :: ${boundaryType} & boundaryCode :: ${boundaryCode} `
        );
        const newRequestBody = JSON.parse(JSON.stringify(request.body));
        activityMessage.push(
          await generateActivityMessage(
    ...
    if (flag === 1) {
      throwError(
        ...
        400,
        "VALIDATION_ERROR",
        "Boundary already present in the system"
      );
    }
    request.body = {
      ...request.body,
    ...
    const errorMessage =
      error.description || "Error while boundary relationship create";
