...
def clear_scroll(scroll_id):
    """
    Release an Elasticsearch scroll context as soon as the caller is done with it.

    WHY THIS MATTERS
    ----------------
    A scroll context pins segment readers (and the heap/file handles behind
    them) on every data node the search touched. Contrary to what the code in
    these reports assumed, ES does NOT free the context when the scroll is
    exhausted - it lingers until the ?scroll=Nm keep-alive expires. So a report
    that scrolls with scroll=10m and then exits keeps holding shard resources
    for a further ten minutes with nobody reading them.

    Two concrete consequences:
      1. Node heap is held hostage. Segments that would otherwise be merged
         away cannot be released while a reader is open on them.
      2. Open contexts count against `search.max_open_scroll_context`
         (default 500, applied per node and shared by every client of that
         node). Enough concurrent report runs and new scrolls start failing
         with "Trying to create too many scroll contexts" - not just for these
         reports, but for anything else scrolling against the same node.

    USAGE
    -----
    Always call from a `finally:` so it runs on the break path, the early-exit
    ...
    Pass the MOST RECENT scroll id. ES may hand back a new _scroll_id on each
    page; the latest one addresses the same underlying contexts, so deleting it
    frees everything the scroll was holding.

    Accepts a single id or a list of ids. Never raises and never blocks the
    caller for long - cleanup must not be able to fail a finished report.
    """
    if not scroll_id:
        return False
    scroll_ids = scroll_id if isinstance(scroll_id, (list, tuple, set)) else [scroll_id]
    scroll_ids = [s for s in scroll_ids if s]
    if not scroll_ids:
        return False
    try:
        response = requests.delete(
            f"{ES_HOST}/_search/scroll",
            data=json.dumps({"scroll_id": list(scroll_ids)}),
            headers={"Content-Type": "application/json", "Authorization": encoded_es_password},
            verify=False,
            timeout=30,
        )
        # 404 just means the context already expired - that is a success for us.
        if response.status_code in (200, 404):
            return True
        print(f"Could not clear scroll context (HTTP {response.status_code})")
    except Exception as e:
        # Never let cleanup failure take down a report that already succeeded.
        print(f"Could not clear scroll context ({type(e).__name__}: {e})")
    return False
