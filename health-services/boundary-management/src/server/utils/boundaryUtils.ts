...
import { getSheetData ,getAutoGeneratedBoundaryCodesHandler,createBoundaryEntities
  ,createBoundaryRelationship,createExcelSheet,createAndUploadFile,stripInvisibleMarks
...
async function buildExistingSeedFromDb(request: any): Promise<{ mappingMap: Map<any, string>, countMap: Map<any, number>, entityNamedKeySet: Set<string> }> {
  ...
  try {
    ...
    const walk = (nodes: any[], parentPath: string, parentElem: any) => {
      (nodes || []).forEach((b: any) => {
        if (!b?.code) return;
        // Symmetric normalization: the sheet side strips invisible bidi/zero-width marks at parse
        // (getRawCellValue -> stripInvisibleMarks), so the seed name must be stripped identically.
        // Otherwise a stored name carrying such a mark (e.g. ingested by a pre-strip build) would
        // never match the stripped sheet row and would be re-minted as a duplicate. This is a no-op
        // for names without such marks, so healthy round-trips are byte-for-byte unaffected.
        const name = stripInvisibleMarks(codeToName.get(b.code) ?? b.code);
