...
import { getSheetData ,getAutoGeneratedBoundaryCodesHandler,createBoundaryEntities
  ,createBoundaryRelationship,createExcelSheet,createAndUploadFile,stripInvisibleMarks,normalizeApostrophes
...
async function buildExistingSeedFromDb(request: any): Promise<{ mappingMap: Map<any, string>, countMap: Map<any, number>, entityNamedKeySet: Set<string> }> {
  ...
  try {
    ...
    // Must match the module the names were WRITTEN to (transformAndCreateLocalisation ->
    // getLocalisationModuleName), which normalizes -/./:/space/ in the hierarchy type to _.
    // Hand-rolling it with the raw hierarchyType made this lookup miss entirely for any hierarchy
    // whose name contains one of those characters (e.g. "LNPERF11-5K", "CHAD Co-Delivery"), so every
    // seed name fell back to the raw code and the tree was re-minted with _1 suffixes.
    const moduleName = getLocalisationModuleName(hierarchyType);
    ...
    const walk = (nodes: any[], parentPath: string, parentElem: any) => {
      (nodes || []).forEach((b: any) => {
        if (!b?.code) return;
        // Symmetric normalization: the sheet side strips invisible bidi/zero-width marks AND folds the
        // apostrophe look-alikes at parse (getRawCellValue -> normalizeApostrophes(stripInvisibleMarks)),
        // so the seed name must be normalised identically and in the same order. Otherwise a stored name
        // carrying such a character (e.g. ingested by an earlier build - the validator has always
        // ALLOWED U+2019 U+2018 U+02CA U+02BC and backtick, so they are present in real data) would
        // never match the normalised sheet row and would be re-minted as a duplicate. This is a no-op
        // for names without such characters, so healthy round-trips are byte-for-byte unaffected.
        const name = normalizeApostrophes(stripInvisibleMarks(codeToName.get(b.code) ?? b.code));
