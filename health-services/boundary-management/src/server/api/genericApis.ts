...
import {boundaryKeyOf,getLocalizedHeaders,throwError,getDataSheetReady} from "../utils/genericUtils";
...
async function getAutoGeneratedBoundaryCodes(boundaryList: any, childParentMap: any, elementCodesMap: any, countMap: any, request: any,hierarchy?:any) {
  ...
  const seenInColumn: Set<string>[] = []; // O(1) per-column structural dedup (was columnsData[i].find -> O(n^2))
  ...
  // (replaces findMapValue's O(n) forEach scan, which made codegen O(n^2)).
  // Insertion order preserves findMapValue's last-match-wins for duplicate structural keys.
  const elementCodesIndex = new Map<string, any>();
...
function generateElementCode(sequence: any, parentElement: any, parentBoundaryCode: any, element: any, excludeBoundaryNameAtLastFromBoundaryCodes?: any, childParentIndex?: Map<string, any>, elementCodesMap?: any) {
  ...
  if (excludeBoundaryNameAtLastFromBoundaryCodes) {
    ...
    // O(1) string-keyed lookup (was findMapValue -> O(n))
    const grandParentElement = childParentIndex ? childParentIndex.get(boundaryKeyOf(parentElement)) : null;
