...
import {findMapValue,boundaryKeyOf,getLocalizedHeaders,throwError,getDataSheetReady} from "../utils/genericUtils";
...
async function getAutoGeneratedBoundaryCodes(boundaryList: any, childParentMap: any, elementCodesMap: any, countMap: any, request: any,hierarchy?:any) {
  ...
  for (let i = 0; i <= lastPresentHierarchyIndex; i++) {
    ...
    const existingElement = columnsData[i].find((existing: any) => boundaryKeyOf(existing) === boundaryKeyOf(element));
    if (!existingElement) {
