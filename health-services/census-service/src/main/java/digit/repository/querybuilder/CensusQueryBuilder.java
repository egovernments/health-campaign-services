...
public class CensusQueryBuilder {
    ...
    private static final String CENSUS_SEARCH_QUERY_COUNT_WRAPPER = "SELECT COUNT(DISTINCT census_id) AS total_count FROM ( ";

    private static final String CENSUS_STATUS_COUNT_WRAPPER = "SELECT COUNT(DISTINCT census_id) as census_status_count, census_status FROM ({INTERNAL_QUERY}) as census_status_map GROUP BY census_status";
    ...
    private String buildCensusQuery(CensusSearchCriteria criteria, List<Object> preparedStmtList, Boolean isCount, Boolean isStatusCount) {
        ...
        if (!CollectionUtils.isEmpty(criteria.getIds())) {
            queryUtil.addClauseIfRequired(builder, preparedStmtList);
            builder.append(" cen.id IN ( ").append(queryUtil.createQuery(criteria.getIds().size())).append(" )");
