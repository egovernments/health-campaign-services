...
class BoundaryCodeResolverTest {
    ...
    // --- write-through repair: the cached map and the durable cell must agree -----------------
    // Root cause of demo 2026-07-30: the row maps are shared/cached across runs while the workbook cell is
    // the durable artifact. A run that re-downloaded the original (blank-cell) file but hit a cached map
    // already carrying codes would skip resolution and never write the cells; a later re-read of the sheet
    // in that same run then produced BLANK codes, which were persisted.

    @Test
    void restoresTheWorkbookCellWhenTheCachedMapAlreadyCarriesTheCode() {
        // Simulate a cache hit from an earlier run: the shared map has the code, the cell is blank.
        List<Map<String, Object>> rows = parsedRows();
        Map<String, Object> mapped = rows.stream()
                .filter(r -> Integer.valueOf(3).equals(r.get(ProcessingConstants.ACTUAL_ROW_NUMBER_KEY)))
                .findFirst().orElseThrow();
        mapped.put(CODE, "MZ_MD");
        assertNull(workbook.getSheet(SHEET).getRow(2).getCell(2), "precondition: cell starts blank");

        resolver.resolveBlankBoundaryCodes(workbook, resource);

        assertEquals("MZ_MD", workbook.getSheet(SHEET).getRow(2).getCell(2).getStringCellValue(),
                "the blank cell must be repaired from the code already present in the shared map");
    }

    @Test
    void neverOverwritesACodeAlreadyPresentInTheCell() {
        resolver.resolveBlankBoundaryCodes(workbook, resource);

        assertEquals("PRE", workbook.getSheet(SHEET).getRow(6).getCell(2).getStringCellValue(),
                "a code already durable in the file is authoritative and must be left untouched");
    }

    @Test
    void repairIsIdempotentAcrossRepeatedRuns() {
        List<Map<String, Object>> rows = parsedRows();
        rows.stream()
                .filter(r -> Integer.valueOf(3).equals(r.get(ProcessingConstants.ACTUAL_ROW_NUMBER_KEY)))
                .findFirst().orElseThrow()
        ...
        assertEquals("MZ_MD", workbook.getSheet(SHEET).getRow(2).getCell(2).getStringCellValue(),
                "a second pass must not disturb the repaired cell");
    }

    @Test
    void unresolvableRowStaysBlankInBothMapAndCell() {
        resolver.resolveBlankBoundaryCodes(workbook, resource);

        Sheet sheet = workbook.getSheet(SHEET);
        org.apache.poi.ss.usermodel.Cell cell = sheet.getRow(4).getCell(2);
        assertTrue(cell == null || cell.getStringCellValue().isEmpty(),
                "an out-of-dropdown selection must stay blank for the validators to flag");
    }
}
