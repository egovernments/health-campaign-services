...
/**
 * Utility for creating a simple single column dropdown with 2nd level boundaries only
 * Shows only the 2nd level localized boundaries without any hierarchy
 */
@Component
@Slf4j
public class HierarchicalBoundaryUtil {
    ...
    /**
     * Adds a simple 2nd level boundary dropdown column to an existing sheet
     * Shows only 2nd level boundaries as plain options
     * 
    ...
    public void addHierarchicalBoundaryColumn(XSSFWorkbook workbook, String sheetName, Map<String, String> localizationMap,
        ...
        // Extract only 2nd level boundaries
        Map<String, String> secondLevelBoundaries = extractSecondLevelBoundaries(
                filteredBoundaries, localizationMap);
        ...
        // Add localized header for 2nd level boundaries
        Cell boundaryHeaderCell = visibleRow.createCell(lastSchemaCol);
        String level2Key = "HCM_CAMP_CONF_LEVEL_2";
        String level2Name = localizationMap.getOrDefault(level2Key, "Select Boundary");
        ...
        createSecondLevelBoundaryDropdown(workbook, secondLevelBoundaries);
        
        // Add data validation for boundary column
        addSecondLevelBoundaryValidation(workbook, sheet, lastSchemaCol);
        
        // Set column widths
        sheet.setColumnWidth(lastSchemaCol, 50 * 256); // Standard width for dropdown
        ...
        log.info("Added 2nd level boundary dropdown with {} entries", secondLevelBoundaries.size());
    }
    
    /**
     * Extracts only 2nd level boundaries from the boundary data
     * Returns a map of localized boundary name to boundary code
     */
    private Map<String, String> extractSecondLevelBoundaries(
            List<BoundaryUtil.BoundaryRowData> boundaries,
            Map<String, String> localizationMap) {
        
        // Use TreeMap for natural sorting of boundary names
        Map<String, String> secondLevelBoundaries = new TreeMap<>();
        ...
        for (BoundaryUtil.BoundaryRowData boundary : boundaries) {
            ...
            if (path.size() > 1 && path.get(1) != null) {
                ...
                if (!processedCodes.contains(boundaryCode)) {
                    processedCodes.add(boundaryCode);
                    String boundaryName = localizationMap.getOrDefault(boundaryCode, boundaryCode);
                    secondLevelBoundaries.put(boundaryName, boundaryCode);
                }
            }
        }
     ...
     */
    private void createSecondLevelBoundaryDropdown(XSSFWorkbook workbook, Map<String, String> boundaryMap) {
        ...
        int rowNum = 0;
        for (String boundaryName : boundaryMap.keySet()) {
        ...
        if (!boundaryMap.isEmpty()) {
            ...
            namedRange.setRefersToFormula("_h_SecondLevelBoundaries_h_!$A$1:$A$" + boundaryMap.size());
        }
        ...
        rowNum = 1;
        for (Map.Entry<String, String> entry : boundaryMap.entrySet()) {
            Row row = codeMapSheet.createRow(rowNum++);
            row.createCell(0).setCellValue(entry.getKey());   // Boundary name
            row.createCell(1).setCellValue(entry.getValue()); // Boundary code
        }
        ...
        if (!boundaryMap.isEmpty()) {
            ...
            codeMapRange.setRefersToFormula("_h_SecondLevelCodeMap_h_!$A$1:$B$" + (boundaryMap.size() + 1));
        }
    }
