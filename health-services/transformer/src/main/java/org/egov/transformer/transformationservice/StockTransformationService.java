...
public class StockTransformationService {
    ...
    public void transform(List<Stock> stocksList, String topic) {
        log.info("transforming for STOCK id's {}", stocksList.stream()
                .map(Stock::getId).collect(Collectors.toList()));
        String indexTopic = transformerProperties.getTransformerProducerBulkStockIndexV1Topic();
        ...
        log.info("transformation success for STOCK id's {}", transformedStockList.stream()
                ...
                .collect(Collectors.toList()));
        producer.push(indexTopic, transformedStockList);

        // Check if this is an update topic and push to new topic with swapped facility fields
        if (topic != null && topic.contains("update")) {
            String updatedIndexTopic = transformerProperties.getTransformerProducerBulkStockIndexUpdatedV1Topic();
            List<StockIndexV1> transformedStockUpdatedList = stocksList.stream()
                    .map(this::transformToUpdatedV1)
                    .collect(Collectors.toList());
            log.info("transformation success for STOCK id's in updated format {}", transformedStockUpdatedList.stream()
                    .map(StockIndexV1::getId)
                    .collect(Collectors.toList()));
            producer.push(updatedIndexTopic, transformedStockUpdatedList);
        }
    }
    ...
    private StockIndexV1 transform(Stock stock) {
        ...
        if (!STAFF.equalsIgnoreCase(facilityType)) {
            ...
            if (facility != null && facility.getAddress() != null && facility.getAddress().getLocality() != null
                ...
                BoundaryHierarchyResult boundaryHierarchyResult = boundaryService.getBoundaryHierarchyWithLocalityCode(facility.getAddress().getLocality().getCode(), tenantId, projectInfo.getHierarchyType());
            ...
            String userProjectId = projectService.getProjectIdFromStaff(facilityId, tenantId);
            if (userProjectId != null) {
                BoundaryHierarchyResult boundaryHierarchyResult = boundaryService.getBoundaryHierarchyWithProjectId(userProjectId, tenantId);
                boundaryHierarchy = boundaryHierarchyResult.getBoundaryHierarchy();
                boundaryHierarchyCode = boundaryHierarchyResult.getBoundaryHierarchyCode();
            }
        }
        ...
        if (boundaryHierarchy.isEmpty() && PROJECT.equalsIgnoreCase(stock.getReferenceIdType().toString())) {
            boundaryHierarchy = boundaryService.getBoundaryHierarchyWithProjectId(stock.getReferenceId(), tenantId).getBoundaryHierarchy();
        }
        ...
        String cycleIndex = projectService.fetchCycleIndexFromProjectAdditionalDetails(tenantId, projectId, projectInfo.getProjectTypeId(), stock.getAuditDetails().getCreatedTime());
        ...
        StockIndexV1 stockIndexV1 = StockIndexV1.builder()
                ...
                .build();
        stockIndexV1.setProjectInfo(projectId, projectInfo);
        return stockIndexV1;
    }

    private StockIndexV1 transformToUpdatedV1(Stock stock) {
        Map<String, String> boundaryHierarchy = new HashMap<>();
        Map<String, String> boundaryHierarchyCode = new HashMap<>();

        String facilityType = getTransactingFacilityType(stock), transactingFacilityType = getFacilityType(stock);
        String transactingFacilityId = getFacilityId(stock), facilityId = getTransactingFacilityId(stock);
        String facilityLevel = null, transactingFacilityLevel = null;
        Long facilityTarget = null;
        String facilityName, transactingFacilityName;
        String tenantId = stock.getTenantId();
        ...
        ObjectNode additionalDetails = objectMapper.createObjectNode();
        additionalDetails.put(CYCLE_INDEX, cycleIndex);
        additionalDetails.put("isUpdated", "true");
        ...
        StockIndexV1 stockIndexV1 = StockIndexV1.builder()
                ...
                .additionalDetails(additionalDetails)
                .build();
        stockIndexV1.setProjectInfo(projectInfo);
        return stockIndexV1;
    }
