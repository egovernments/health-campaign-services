...
class ApplicationService {
  ...
  async getApplications(filter?: {
    ...
    return this.getFilteredApplications(applicationStore, filter);
  }

  getFilteredApplications = (applicationStore: any, filter: any) => {
    let filteredApplications = [];
    if (!filter) {
      filteredApplications = applicationStore;
    } else {
      filteredApplications = applicationStore.filter(
        (app: any) =>
    ...
    return {
      Applications: filteredApplications?.map((app: any) => app?.Application),
    };
  };
  ...
  async updateApplication(
    ...
    logger.info(
      `Application updated: ${JSON.stringify(applicationStore[appIndex])}`
    );
