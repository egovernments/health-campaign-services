...
| `POST /individual/v1/_register` | **New in v2.1** — citizen self-registration and OTP login from Studio (see section 7). |
...
Changes between the `v2.0` baseline and the `master-nigeria-finalpull` release line (merged from *Individual-master-register-studio*), in plain language for product owners, QA and ops.
