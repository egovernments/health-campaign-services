...
**Project parent read-after-write**: `project/v1/_create` returns the id from enrichment, but the row is persisted asynchronously (persister → DB), so a just-created **parent** project can be briefly unsearchable. Same two-layer guard as individual→worker: (1) **throttle the cause** — `runBoundedProject(..., config.project.createPaceDelayMs)` paces the bulk project-create burst between concurrency waves so the persister/DB write queue drains instead of flooding (default `0` = off, tune up under load); (2) **widen the symptom window** — `confirmProjectParentCreation` polls `config.project.confirmRetries` × `confirmPollIntervalMs` (default 15 × 2s ≈ 30s, config-driven, never a hardcoded wait; early-exits on first hit) before throwing `PROJECT_CONFIRMATION_FAILED`. Project-create failure is **hard-blocking**, so a false-negative confirm fails the whole campaign — hence the wide, tunable window.
...
**Polling**: use `pollForTemplateGeneration` / `createAndPollForCompletion` from `utils/pollUtils.ts`. Never remove post-produce waits — intentional eventual-consistency with egov-persister. Post-produce persistence waits go through `getPersistenceWaitTime(count)` in `utils/persistenceWaitUtils.ts` (`PERSISTENCE_WAIT_TIME_MS` flat override via `config.persistenceWaitTimeMs`; unset → `max(5000, count*8)`) — never inline the formula. The helper lives outside `pollUtils.ts` deliberately: `pollUtils` transitively imports `dataManageService → campaignApis → createAndSearch`, which reads `config.host` at import time and breaks any test that partially mocks `../config`.
