...
async function projectCreate(projectCreateBody: any, request: any) {
  ...
  if (!request.body.boundaryProjectMappingForProjectResourceCreation) {
    request.body.boundaryProjectMappingForProjectResourceCreation = {};
  ...
  if (projectCreateResponse?.Project[0]?.id) {
    ...
    if (!request.body.boundaryProjectMappingForProjectResourceCreation[projectCreateBody?.Projects?.[0]?.address?.boundary]) {
      request.body.boundaryProjectMappingForProjectResourceCreation[projectCreateBody?.Projects?.[0]?.address?.boundary] = {};
    }
    ...
    request.body.boundaryProjectMappingForProjectResourceCreation[projectCreateBody?.Projects?.[0]?.address?.boundary].projectId = projectCreateResponse?.Project[0]?.id
  }
  else {
