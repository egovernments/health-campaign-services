...
export function getJsonDataBasedOnUniqueKey(
  sheetData: any,
  getRow = false,
  getSheetName = false,
  sheetName = "sheet1"
  ...
  const primaryHeaders = sheetData[0];  // First row
  const secondaryHeaders = sheetData[1]; // Second row used as final header with fallback

  // Build headers with fallback from primaryHeaders
  const headers = secondaryHeaders.map((val: any, index: number) =>
    val === undefined || val === "" ? primaryHeaders[index] : val
  );
