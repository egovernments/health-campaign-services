...
import { getCampaignNumber, createAndUploadFile, createExcelSheet, getAutoGeneratedBoundaryCodesHandler, createBoundaryEntities, getMDMSV1Data, getTargetSheetDataAfterCode, callMdmsTypeSchema, getSheetDataFromWorksheet, getTargetWorkbook, createAndUploadJsonFile, createBoundaryRelationship } from "../api/genericApis";
...
async function processFetchMicroPlan(request: any){
    ...
    const processedFacilitySheetData = await getSheetData(fileResponse.fileStoreIds[0].url, localizedFacilityTab, false, undefined, localizationMap);
    const processedBoundaryData = await getSheetData(fileResponse.fileStoreIds[0].url, boundaryTab, false, undefined, localizationMap);
