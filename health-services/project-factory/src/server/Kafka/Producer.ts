...
producer.on('ready', () => {
    console.log('Producer is ready'); // Log message indicating producer is ready
});

// Event listener for 'error' event, indicating that the producer encountered an error
producer.on('error', (err) => {
    console.log('Producer is in error state'); // Log message indicating producer is in error state
    console.log(err.stack || err); // Log the error stack or message
});
