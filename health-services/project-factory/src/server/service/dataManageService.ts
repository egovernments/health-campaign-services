...
import { getCampaignSearchResponse, processGenericRequest } from "../api/campaignApis";
import { createAndUploadFile, getBoundarySheetData } from "../api/genericApis";
import { checkIfSourceIsMicroplan, getLocalizedName, getResourceDetails, processDataSearchRequest } from "../utils/campaignUtils";
...
const downloadDataService = async (request: express.Request) => {
    ...
    if (
        ...
        if(campaignId == "microplan") isSourceMicroplan = true;
        let campaignResponse = await getCampaignSearchResponse(request);
        let campaignObject = campaignResponse?.CampaignDetails?.[0];
        if (!campaignObject) {
            logger.warn(`No campaign found for the given Campaign Id ${request?.query?.campaignId}`);
        }
        else if(checkIfSourceIsMicroplan(campaignObject)){
            isSourceMicroplan = true;
        }
        // Throw error if data is not found
        // const newRequestToGenerate = buildGenerateRequest(request);
        if (isSourceMicroplan) {
            await handleAutoGenerateForMicroplan(request, campaignId, type);
        } else {
            await handleAutoGenerateDefault(request, type, campaignId);
        }
    }
...
async function handleAutoGenerateForMicroplan(request: any, campaignId: string, type: string) {
    const newRequestToGenerate = buildGenerateRequest(request);
    logger.info(`Triggering auto generate for microplan since no resources got generated for the given Campaign Id ${campaignId} & type ${type}`);
    await callGenerate(newRequestToGenerate, type);
}

async function handleAutoGenerateDefault(request: any, type: string, campaignId: string) {
    ...
    logger.info(`Triggering auto generate since no resources got generated for the given Campaign Id ${campaignId} & type ${type}`);
    await triggerGenerate(type, tenantId, hierarchyType, campaignId, useruuid, locale);
}
