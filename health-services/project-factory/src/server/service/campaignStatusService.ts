...
async function getCampaignStatusService(
    ...
    try {
        ...
        // 1. Mapping Data Query
        const mappingQuery = `
            SELECT type, status, COUNT(status) AS status_count
            FROM ${mappingTableName}
            WHERE campaignnumber = $1 AND tenantid = $2
            GROUP BY type, status
        `;
        const mappingResult = await executeQuery(mappingQuery, [campaignNumber, tenantId]);

        // 2. Campaign Data Query
        const campaignQuery = `
            SELECT type, status, COUNT(status) AS status_count
            ...
            GROUP BY type, status
        `;
        const campaignResult = await executeQuery(campaignQuery, [campaignNumber, tenantId]);

        // 3. Process Data Query
        const processQuery = `
            SELECT processname, status
            FROM ${processTableName}
            WHERE campaignnumber = $1 AND tenantid = $2
        `;
        const processResult = await executeQuery(processQuery, [campaignNumber, tenantId]);
