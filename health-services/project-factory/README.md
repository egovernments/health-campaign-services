...
- **Failed campaigns are resumable, and user creation is retry-safe.** A failed campaign can be corrected and re-run; already-completed rows are skipped and only pending/failed rows are retried. User creation specifically now skips workers already in HRMS, tolerates partial-batch failures without failing the whole campaign, keeps a per-row error history, and reconciles a worker whose phone already exists in HRMS to a completed `EXISTING` row (see §6 Failure / Retry Handling).
...
- **User creation is safely retryable (v2.1).** Before sending a worker to HRMS, project-factory checks whether the phone already exists; if so it reuses that account instead of failing. A partial batch no longer aborts the whole run, and each failed row keeps a short attempt history so operators can tell a transient blip apart from a real data problem. The per-user HRMS fallback (used when the bulk create can't be used) now runs in bounded-concurrency windows and **retries transient failures** — e.g. `Failed to obtain JDBC Connection` under DB-pool pressure — with backoff, instead of firing the whole batch at once and exhausting the downstream pool; permanent errors (already-exists / validation) are not retried.
- **Adopted-user reconciler (v2.1).** A `pending` user row whose phone already exists in HRMS is now driven to *completed* — its sheet status is set to `EXISTING` and the worker's **real existing login id** is surfaced on the credentials sheet (adopted users get no new password). This lets a partially-created campaign converge to zero pending rows instead of the completion poller timing out waiting on rows that can never be freshly created.
- **Message processing is at-least-once, and handlers are guarded for redelivery (v2.1).** Kafka offsets now commit only *after* a handler finishes, so a pod restart, crash or rebalance mid-processing **redelivers** the message instead of silently dropping the in-flight work (previously it was fire-and-forget "at-most-once"). Consumer parallelism is bounded by `KAFKA_CONSUMER_MAX_CONCURRENT` (default 5). To stay safe under redelivery, the facility batch re-reads live DB status and creates only not-yet-completed rows, the task and legacy mapping-task short-circuit if their process is already `completed`, and per-boundary project creation adopts an existing project for the same boundary + `campaignNumber` before creating. A *handled* failure (row marked `failed`) still commits and is not redelivered — only a crash triggers redelivery — so there is no poison-message reprocessing loop.
- **Idempotency caveat.** Bulk creates are not uniformly idempotent — a network blip after a downstream commit but before the response can cause a duplicate attempt on retry. The persister deduplicates on `campaignNumber`; the guards above cover the redelivery path, but verify per-stage when adding new orchestration.
