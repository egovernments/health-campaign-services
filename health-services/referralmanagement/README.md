...
- **Bulk batches survive within-batch duplicates.** A duplicate `clientReferenceId` inside one bulk batch no longer fails the whole batch — the first is kept, each later duplicate is flagged individually, and the good records still persist (at-least-once behaviour). Structural validators also reject a referral/side-effect that is missing its own `clientReferenceId` or its beneficiary/task link (`REQUIRED_LINK_MISSING`) — gated by `referralmanagement.required.link.validation`, which defaults to `false` for backward compatibility.
...
- **Structural link fields are enforced when `referralmanagement.required.link.validation` is `true` (default `false`).** With the flag enabled, a referral must carry a `clientReferenceId` and a beneficiary link (`projectBeneficiaryId` or `projectBeneficiaryClientReferenceId`); a side-effect must carry a `clientReferenceId` and a task link (`taskId` or `taskClientReferenceId`). Missing any of these rejects the record with error code `REQUIRED_LINK_MISSING`. This is a presence check, not an existence check, so it is not governed by `referralmanagement.relationship.validation` — it has its own flag, defaulted `false` because the old service accepted standalone referrals and side-effects.
