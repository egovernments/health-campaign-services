...
import org.egov.project.util.NoResourceTaskStatus;
...
public class PtIsResouceEmptyValidator implements Validator<TaskBulkRequest, Task> {
    ...
    @Autowired
    private NoResourceTaskStatus noResourceTaskStatus;
    /**
    ...
    public Map<Task, List<Error>> validate(TaskBulkRequest request) {
        ...
        if(!entities.isEmpty()) {
            entities.forEach(task -> {
                if (CollectionUtils.isEmpty(task.getResources()) && !noResourceTaskStatus.isExists(
                    task.getStatus())) {
                    /**
                    ...
                    String errorMessage = ProjectConstants.TASK_NOT_ALLOWED_RESOURCE_CANNOT_EMPTY_ERROR_MESSAGE
                            + String.join(ProjectConstants.OR, noResourceTaskStatus.getStatusSet());
                ...
                } else if (!CollectionUtils.isEmpty(task.getResources()) && noResourceTaskStatus.isExists(task.getStatus())) {
                    /**
