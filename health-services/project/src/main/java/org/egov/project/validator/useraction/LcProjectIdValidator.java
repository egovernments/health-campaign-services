...
/**
 * LcProjectIdValidator is responsible for validating the Project IDs in UserActionBulkRequest.
...
public class LcProjectIdValidator implements Validator<UserActionBulkRequest, UserAction> {
    ...
    /**
     * Constructor to initialize ProjectRepository
     *
     * @param projectRepository the repository to check project IDs against
     */
    @Autowired
    ...
    public Map<UserAction, List<Error>> validate(UserActionBulkRequest request) {
        log.info("Starting validation for project IDs in UserActionBulkRequest with {} entities", request.getUserActions().size());
        ...
        log.info("Completed validation for project IDs in UserActionBulkRequest");
