...
Changes between the `v2.0` baseline and the `master-nigeria-finalpull` release line. For this service the window is **dependency / plumbing only — no functional, API, or DB-migration changes.**
...
| Field | Value |
|---|---|
| Release | **v2.1** (`master-nigeria-finalpull`) — service artifact `1.1.1-SNAPSHOT` |
